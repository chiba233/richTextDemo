...
import type { TranslationCopy, ComposedState } from "../types";
...
const mode = ref<"structural" | "richText" | "diff">("structural");
...
const structuralJson = computed(() => safeStringify(props.composedState.structuralTree));
const richTextJson = computed(() => safeStringify(props.composedState.richTextTokens));
...
<template>
  <div class="inspect-overlay" @click.self="emit('close')">
    <section class="inspect-modal glass" role="dialog" aria-modal="true">
      <header class="inspect-head">
        <h2>{{ copy.previewEyebrow }}</h2>
        <button type="button" class="inspect-close" aria-label="close" @click="emit('close')">
          ×
        </button>
      </header>
      ...
      <div class="inspect-tabs">
        <button
          ...
          :class="{ 'preview-mode-tab-active': mode === 'structural' }"
        ...
        <button
          type="button"
          class="preview-mode-tab"
          :class="{ 'preview-mode-tab-active': mode === 'richText' }"
          @click="mode = 'richText'"
        >
        ...
        <button
          type="button"
          class="preview-mode-tab"
          :class="{ 'preview-mode-tab-active': mode === 'diff' }"
          @click="mode = 'diff'"
        >
      ...
      <div class="inspect-body">
        <div v-if="mode === 'structural'" class="structural-split">
          <section class="structural-pane">
        ...
        <section v-else-if="mode === 'richText'" class="modal-section">
          <div class="slice-preview-head">
        ...
        <section v-else class="modal-section">
          ...
          <div v-else class="empty-state">{{ copy.diffEmpty }}</div>
        </section>
      </div>
    </section>
  </div>
